...
#define         KEY4X4_COL1          GPIO_PinRead(GPIOB,10)    
#define         KEY4X4_COL2          GPIO_PinRead(GPIOB,2)    
#define         KEY4X4_COL3          GPIO_PinRead(GPIOB,1)    
#define         KEY4X4_COL4          GPIO_PinRead(GPIOB,0)    
...
#define         KEY4X4_ROW4_UP         GPIO_PinWrite(GPIOB,11,1)//
...
#define         KEY4X4_ROW4_DOWN         GPIO_PinWrite(GPIOB,11,0)//
...
// Init I2C
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);
	//SCL of I2C1
	GPIO_PinConfigure(GPIOB, 6,
                      GPIO_AF_OPENDRAIN, 
                      GPIO_MODE_OUT50MHZ);
	//SDA of I2C1
	GPIO_PinConfigure(GPIOB, 7,
                      GPIO_AF_OPENDRAIN, 
                      GPIO_MODE_OUT50MHZ);
	//I2C_InitTypeDef I2CInit;
...
uint8_t EEPROM_W_Regs(uint16_t addr,uint16_t length,uint8_t* pdata)
{
	...
	while(I2C_GetFlagStatus(I2C1, I2C_FLAG_ADDR) == RESET)
	{
		...
		/* clear addr flag */
	temp = I2C1->SR2;
	I2C_SendData(I2C1, addr>>8);//8 bit dau
		/* clear addr flag */
	temp = I2C1->SR2;
	//while ((I2C1->SR1 & 0x00004) != 0x000004);
	I2C_SendData(I2C1, addr&0xff);//8 bit sau
	for(i=0;i<length;i++){
	temp = I2C1->SR2;
	while ((I2C1->SR1 & 0x00004) != 0x000004);
...
void EEPROM_init(void)
{
	...
	I2C_Send7bitAddress(I2C1,EEPROM_addr,I2C_Direction_Transmitter); // Send EEPROM_addr slave address
...
//                      GPIO_MODE_INPUT);
   ...
   j = GPIO_ReadInputDataBit(GPIOA , GPIO_Pin_8);
