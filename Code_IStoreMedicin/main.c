...
#define IND_COM_OPP   GPIO_PinConfigure(GPIOA, 11,GPIO_OUT_PUSH_PULL,GPIO_MODE_OUT50MHZ);GPIO_PinWrite(GPIOB,11,0)// Output Push Pull wait for send data over USART                 
#define IND_COM_IPU   GPIO_PinConfigure(GPIOA, 11,GPIO_IN_PULL_UP,GPIO_MODE_INPUT)// Input Pull Up wait for having data over USART
#define IND_COM_PIN   GPIO_PinRead(GPIOA,11) // PIN indicate data for reciver and send data

#define RS458_SEND   GPIO_PinConfigure(GPIOA, 8,GPIO_OUT_PUSH_PULL,GPIO_MODE_OUT50MHZ);GPIO_PinWrite(GPIOB,8,1)// Output Push Pull wait for send data over USART                 
#define RS485_REC   GPIO_PinConfigure(GPIOA, 8,GPIO_IN_PULL_UP,GPIO_MODE_INPUT);GPIO_PinWrite(GPIOB,8,0)// Input Pull Up wait for having data over USART
//#define IND_COM_PIN   GPIO_PinRead(GPIOA,8) // PIN indicate data for reciver and send data
...
//	uint8_t count=1;
	...
	GLCD_GPIO_init();
	RS458_SEND;
	USART1_SendStr_(">Master Board HWv2 fwv1\r\n");
